...
export default function TypeScriptModule() {
  ...
  // Navigate section-by-section (e.g. 1-8 -> 9-13)
    ...
    <div className="relative flex min-h-0 w-full flex-1 flex-col gap-2 md:flex-row md:gap-3 lg:gap-4">
      ...
      `}>
        ...
        />
          <button
            type="button"
            className="mt-2 flex w-full items-center justify-center gap-2 rounded-lg border-2 border-blue-300 bg-blue-50 px-3 py-2 text-xs font-bold text-blue-900 hover:bg-blue-100 sm:text-sm"
            onClick={() => {
              document.getElementById("typescript-command-recap")?.scrollIntoView({ behavior: "smooth", block: "start" });
              if (window.innerWidth < 768) setSidebarOpen(false);
            }}
          >
            <span aria-hidden>📋</span>
            TypeScript command list
          </button>
      </div>
      ...
      <div className="relative mt-0 flex min-h-0 min-w-0 flex-1 basis-0 flex-col md:mt-0">
        <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden overscroll-contain px-0 lg:min-w-0">
          <CommandRecapPanel
            ...
            variant="blue"
            title="TypeScript command recap"
            subtitle="CLI commands for the TypeScript compiler, project setup, and day-to-day checks."
            groups={typescriptCommandGroups}
          />
          {renderContent()}
        </div>
