...
import { nextCommandGroups } from "../data/techCommandRecaps";
...
function ConceptCard({
  ...
  return (
    <div 
      ...
      className="w-full max-w-full scroll-mt-4 flex flex-col overflow-hidden rounded-2xl border border-zinc-200/80 border-l-4 border-l-indigo-500 bg-white p-4 shadow-sm ring-1 ring-zinc-100 sm:p-5 md:p-6"
    >
      <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-3 sm:mb-4 w-full">
        <span className="text-xl sm:text-2xl font-bold text-indigo-600 flex-shrink-0">#{number}</span>
      ...
      {children && (
        <div className="mt-6 w-full max-w-full overflow-visible">
          <button
            ...
            className="z-10 flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-md shadow-indigo-600/20 transition-colors duration-200 hover:bg-indigo-700 sm:px-4 sm:py-3 sm:text-base"
            type="button"
          >
            <span className="font-semibold text-white">{showExamples ? "Hide Examples" : "See Examples"}</span>
            <span className="text-lg text-white sm:text-xl">{showExamples ? "▲" : "▼"}</span>
          </button>
          {showExamples && (
...
export default function NextJSModule() {
  ...
  // Navigate section-by-section (1-5 -> 6-10 -> 11-15 ...)
    ...
    <div className="relative flex min-h-0 w-full flex-1 flex-col gap-2 md:flex-row md:gap-3 lg:gap-4">
      ...
      `}>
        <div className="mb-2 sm:mb-3 pb-2 sm:pb-3 border-b border-zinc-200/80">
          ...
          <button
            type="button"
            className="mt-2 flex w-full items-center justify-center gap-2 rounded-lg border-2 border-violet-300 bg-violet-50 px-3 py-2 text-xs font-bold text-violet-900 hover:bg-violet-100 sm:text-sm"
            onClick={() => {
              document.getElementById("nextjs-command-recap")?.scrollIntoView({ behavior: "smooth", block: "start" });
              if (window.innerWidth < 768) setSidebarOpen(false);
            }}
          >
            <span aria-hidden>📋</span>
            Next.js command list
          </button>
        </div>
      ...
      <div className="relative mt-0 flex min-h-0 min-w-0 flex-1 basis-0 flex-col md:mt-0">
        <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden overscroll-contain px-0 lg:min-w-0">
          <CommandRecapPanel
            id="nextjs-command-recap"
            variant="violet"
            title="Next.js command recap"
            subtitle="create-next-app, dev/build/start, and common CLI workflows."
            groups={nextCommandGroups}
          />
        ...
        {/* Topic Navigation Buttons - Bottom */}
        {activeConcept && (
